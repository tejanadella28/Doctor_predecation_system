...
# Load datasets
sym_des = pd.read_csv("dataset/symtoms.csv")
...
# Helper function to get details based on predicted disease
def helper(dis):
    desc = description.loc[description['Disease'] == dis, 'Description'].values
    desc = desc[0] if len(desc) > 0 else "No description available"

    pre = precautions.loc[
        precautions['Disease'] == dis, ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pre = pre.values.flatten().tolist() if not pre.empty else []

    med = medications.loc[medications['Disease'] == dis, 'Medication'].values.tolist()
    die = diets.loc[diets['Disease'] == dis, 'Diet'].values.tolist()
    wrkout = workout.loc[workout['disease'] == dis, 'workout'].values.tolist()
...
def get_predicted_value(patient_symptoms):
    ...
    for symptom in patient_symptoms:
        symptom = symptom.lower().strip().replace(" ", "_")  # Normalize input
        if symptom in symptoms_dict:
            input_vector[symptoms_dict[symptom]] = 1
        else:
            return f"Invalid Symptom: {symptom}"  # Return first invalid symptom

    input_vector = np.array([input_vector])  # Ensure correct shape for model input
